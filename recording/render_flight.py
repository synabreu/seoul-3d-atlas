...
void main(){
 ...
 vec3 light=normalize(vec3(-25.0,42.0,12.0)-world);
 float diffuse=max(dot(n,light),0.0);
 vec3 ambient=mix(vec3(0.48,0.55,0.43),vec3(0.72,0.80,0.88),n.y*0.5+0.5);
 vec3 rgb=color*(ambient*0.92+vec3(1.0,0.93,0.80)*diffuse*0.63);
 ...
 frag=vec4(mix(rgb,vec3(230.0,237.0,231.0)/255.0,fog),opacity);
}
'''
...
for item in data['objects']:
 ...
 item['mode_value']={'triangles':moderngl.TRIANGLES,'lines':moderngl.LINES,'line_strip':moderngl.LINE_STRIP}[item['mode']]
 objects.append(item)
...
text(dr,(36,47),'서울',42);text(dr,(133,70),'SEOUL',20);text(dr,(225,71),'3D',13,accent);text(dr,(255,71),'v1.3',12,muted)
...
text(dr,(W/2,H-140),'드래그 회전  ·  휠 확대  ·  오른쪽 드래그 이동',12,muted,'mm')
...
def render(frame):
 ...
 fbo.use();fbo.clear(230/255,237/255,231/255,1,depth=1)
 ...
 for item in objects:
  if item.get('bounds'):
   b=item['bounds'];c=np.array(b['center']);
   if np.any(planes[:,:3]@c+planes[:,3]<-b['radius']):continue
  model=item['model']
  if item['motion']>=0:model=motion[frame,item['motion']].T@model
  program['model'].write(model.T.astype('f4').tobytes());program['base_color'].value=tuple(item['color']);program['roughness'].value=item['roughness'];program['opacity'].value=item['opacity']
